...
const ProyectosMegaMenu: React.FC<ProyectosMegaMenuProps> = ({ subCategories, featuredImageUrl, onSelectProjectCategory, onClose }) => {
      ...
      <div className="grid grid-cols-4 gap-6 py-5">
        <div className="col-span-3 grid grid-cols-6 gap-x-4 gap-y-3">
          {subCategories.map((category) => (
            <a href="#" key={category.id} onClick={(e) => handleClick(e, category.name)} className="group flex flex-col items-center text-center p-1 rounded-lg hover:bg-gray-50 transition-colors duration-200">
        ...
        <div className="col-span-1 relative rounded-md overflow-hidden group h-full">
          <img src={featuredImageUrl} alt="Plano de diseño arquitectónico para un proyecto de Decora Group" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
