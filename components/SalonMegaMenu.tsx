...
import { SALON_SUB_CATEGORIES } from '../constants';

interface SalonMegaMenuProps {
  onSelectCategory: () => void;
}

const SalonMegaMenu: React.FC<SalonMegaMenuProps> = ({ onSelectCategory }) => {
  return (
    <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-4 gap-6 py-5">
        ...
        <div className="col-span-1 relative rounded-md overflow-hidden group h-full">
          ...
          <a href="#" onClick={(e) => { e.preventDefault(); onSelectCategory(); }} className="absolute inset-0" aria-label="Ver todos los muebles de salón"></a>
        </div>
      </div>
    </div>
  );
