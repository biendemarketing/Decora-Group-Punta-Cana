...
import { DORMITORIO_SUB_CATEGORIES } from '../constants';

interface DormitorioMegaMenuProps {
  onSelectCategory: () => void;
}

const DormitorioMegaMenu: React.FC<DormitorioMegaMenuProps> = ({ onSelectCategory }) => {
  return (
    <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-4 gap-6 py-5">
        <div className="col-span-3 grid grid-cols-6 gap-x-4 gap-y-3">
          {DORMITORIO_SUB_CATEGORIES.map((category) => (
        ...
        <div className="col-span-1 relative rounded-md overflow-hidden group h-full">
          <img src="https://img.furniture1.eu/v7/_f1_/images/desktop_menu/menu-bedroom-side-image.jpg" alt="Dormitorio elegante con cama grande y decoración minimalista" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
          <div className="absolute inset-0 bg-black bg-opacity-40 flex items-end p-4">
            <h3 className="text-white text-lg font-bold">Muebles de dormitorio</h3>
          </div>
          <a href="#" onClick={(e) => { e.preventDefault(); onSelectCategory(); }} className="absolute inset-0" aria-label="Ver todos los muebles de dormitorio"></a>
        </div>
      </div>
    </div>
  );
