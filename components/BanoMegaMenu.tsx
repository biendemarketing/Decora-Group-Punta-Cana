...
const BanoMegaMenu: React.FC<BanoMegaMenuProps> = ({ subCategories, featuredImageUrl, onSelectCategory, onClose }) => {
    ...
    <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-4 gap-6 py-5">
        ...
        <div className="col-span-1 relative rounded-md overflow-hidden group h-full">
          <img src={featuredImageUrl} alt="Baño moderno con mueble de lavabo flotante" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
          <div className="absolute inset-0 bg-black bg-opacity-40 flex items-end p-4">
            <h3 className="text-white text-lg font-bold">Muebles de baño</h3>
          </div>
          <a href="#" onClick={handleClick} className="absolute inset-0" aria-label="Ver todos los muebles de baño"></a>
        </div>
      </div>
    </div>
  );
