...
const PrintCataloguePage: React.FC<PrintCataloguePageProps> = ({ catalogue, products, navigationData }) => {
  ...
  // --- Intelligent Page Generation Logic ---
  ...
  Object.entries(productsByCategory).forEach(([category, categoryProducts]) => {
      ...
      // Try to fill the last page from the previous category
      ...
      // Create new pages for the remaining products of the current category
  ...
  // Add the last, possibly incomplete, page
  ...
  pages.forEach((page, index) => {
      if (!categoryIndex[page.title]) {
          categoryIndex[page.title] = index + 3; // 1:Cover, 2:Index, pages start at 3
      }
  });
  ...
  const CoverPage = () => {
    ...
    if (!template) return <div className="page">Plantilla no encontrada</div>;
  ...
  const ProductPage = ({ pageData, pageNumber }: { pageData: {title: string, products: Product[]}, pageNumber: number }) => {
            ...
            pageContent = (
                <div className="space-y-4">
                    {productsToShow.map(product => (
                        <div key={product.id} className="flex gap-4 border-b pb-4 break-inside-avoid">
                            <div className="w-32 h-32 bg-gray-100 rounded-md overflow-hidden flex items-center justify-center flex-shrink-0">
                                <img src={product.imageUrl} alt={product.name} className="max-w-full max-h-full object-contain rounded"/>
                            </div>
                            <div>
                                <h4 className="font-bold text-lg text-[#5a1e38]">{product.name}</h4>
                                <p className="text-sm text-gray-600 mt-1">{product.description}</p>
                                {!config.hidePrices && <p className="font-semibold mt-2 text-lg text-gray-800">{formatPrice(product.price)}</p>}
                            </div>
                        </div>
                    ))}
                </div>
        ...
        case 'collage':
             pageContent = (
                <div className="grid grid-cols-3 grid-rows-2 gap-4 h-full">
                    {productsToShow[0] && <div className="col-span-2 row-span-2 h-full"><ProductCard product={productsToShow[0]}/></div>}
                    {productsToShow[1] && <div className="col-span-1 row-span-1 h-full"><ProductCard product={productsToShow[1]}/></div>}
                    {productsToShow[2] && <div className="col-span-1 row-span-1 h-full"><ProductCard product={productsToShow[2]}/></div>}
                </div>
            );
            break;
        case 'grid':
    ...
    return (
        <div className="page p-12 relative flex flex-col">
            <h3 className="text-xl font-bold text-gray-700 mb-6 flex-shrink-0">{title}</h3>
            <div className="flex-grow">
                {pageContent}
            </div>
            {config.showPageNumbers && <div className="page-footer">{pageNumber}</div>}
        </div>
    );
  }

  const BackCoverPage = () => (
    <div className="page flex p-0" style={{backgroundColor: '#5a1e38'}}>
        <div className="w-2/3 bg-white p-12 flex flex-col justify-between">
             <div>
              ...
              <div>
                {config.backCover.showSocials && (
                    <div className="flex items-center gap-4 text-[#5a1e38]">
                        <Instagram size={20} />
                        <Facebook size={20} />
                        <span className="font-semibold">@decoragroup.pc</span>
                    </div>
                )}
              </div>
        </div>
